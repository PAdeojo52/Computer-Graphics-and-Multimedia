...
class Ball {
  ...
  //velocity.limit(1);
  //acceleration.y = acceleration.y + random (-0.001, 0.001);
  //acceleration.x = acceleration.x -0.0001;
  if(location.x>height/4)
  {
    
    acceleration.y = 0.001 + acceleration.y/2;
  ...
  void checkEdges() {
    ...
    else if(location.y < ballRadius/2)
    {
      ...
      //acceleration.y = acceleration.y +0.05;
      //acceleration.x = acceleration.x +0.05;
